import { copyTire, deleteTire } from '@entities/tires/api'
import Table from '@pages/tires/Table'
...
function Tires() {
	document.title = 'CarsDB - Шины'
	const [searchParams, setSearchParams] = useSearchParams()
	const page = Number(searchParams.get('page')) || 1
	const size = Number(searchParams.get('size')) || 10
	const refresh = searchParams.get('refresh')
	const { data: tires, loading, refetch } = useAllDirectory(`${BASE_URL}${API_PATHS.tires}`, 'items', size)
	...
	const handleConfirmDelete = async () => {
		try {
			...
			await deleteTire(deleteId, token)
			setAllTires(allTires.filter(t => t.id !== deleteId))
			setToastText('Шины успешно удалены')
			setShowToast(true)
		} catch {
	...
	const handleCopy = async (tire) => {
		try {
			const token = Cookies.get('token')
			const baseTitle = getBaseTitle(tire.title)
			const titles = allTires.map(t => t.title)
			...
			await copyTire({ ...tire, title: newTitle }, token)
	...
	const tireToDelete = allTires.find(t => t.id === deleteId)
	const subtext = tireToDelete ? `Шины "${tireToDelete.title}" будут удалены навсегда. Это действие нельзя будет вернуть.` : ''
