import TableActions from '@shared/Table/Actions'
import TableGeneric from '@shared/Table/Table'
import PropTypes from 'prop-types'
import { useMemo } from 'react'

function Table({ wheelWidths, loading, onDelete, onCopy }) {
	const columns = useMemo(() => [
		{ Header: 'Ширина', accessor: 'title' },
		{
			Header: 'Действия',
			id: 'actions',
			Cell: ({ row }) => <TableActions onCopy={() => onCopy(row.original)} onDelete={() => onDelete(row.original.id)} />
		}
	], [onDelete, onCopy])

	return (
		<TableGeneric
			data={wheelWidths}
			loading={loading}
			columns={columns}
			addLink="/wheel-width/new"
			addLabel="Добавить"
			lengthId="wheel_widths_length"
			filterId="wheel_widths_filter"
		/>
	)
}

Table.propTypes = {
	wheelWidths: PropTypes.array.isRequired,
	loading: PropTypes.bool,
	onDelete: PropTypes.func.isRequired,
	onCopy: PropTypes.func.isRequired,
}

export default Table
